class Product < ApplicationRecord
  ...
  def searchProduct searchParams
    ...
    # where文を付与する
    where = searchParams.key?('product_name') && 
    searchParams.key?('description') && 
    searchParams.key?('min_price') && 
    searchParams.key?('max_price') ? " where" : ""

    sql << where

    # sqlを生成する
    searchParams.each do | key, val |
        sql << " #{key} like ? "
    end

    # sql実行
    # return Product.find_by_sql([sql, 1, 10])
